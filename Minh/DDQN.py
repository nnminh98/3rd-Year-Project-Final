...
class DDQN:
    def __init__(
        ...
        self.env = SinglePacketRoutingEnv(nodes=nodes1, edges=links1)
    ...
    def training(self, save=False):
        ...
        with tf.Session() as sess:
            ...
            while episode < self.num_episodes:
                ...
                iteration += 1
        rewards = [np.average(self.reward_history[index * self.reward_step:(index + 1) * self.reward_step]) for index in
                   range(int(episode / self.reward_step))]
        print(rewards)
