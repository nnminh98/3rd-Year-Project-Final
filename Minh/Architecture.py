from SimComponents import PacketGenerator, PacketSink, SwitchPort, PortMonitor, Packet, AcknowledgementPacket
...
class Network(object):
    ...
    def add_packet(self, pkt):
        """Function for adding a packet object into the network manually, putting it into the correct src node
    ...
    def packets_received(self):
        """Count all packets that has terminated
        :return:
        """
        received = 0
        for node in self.nodes.values():
            received += node.packet_sink.packets_rec
...
class Link(object):
    ...
    def get_counter_id(self):
        counter_id = self.id.split("_")
        return "{}_{}".format(counter_id[1], counter_id[0])
