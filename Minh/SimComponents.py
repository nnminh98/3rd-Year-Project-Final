...
class Packet(object):
    ...
    def __init__(self, time, size, id, src="a", dst="z", flow_id=0):
        ...
        self.total_hops_travelled = 0
        self.total_route_weight = 0
        self.intermediate_hops_travelled = 0
        self.intermediate_route_weight = 0
    ...
    def increment_hops(self):
        """Increment the number of hops the packet has completed"""
        self.total_hops_travelled += 1
        self.intermediate_hops_travelled += 1

    def incrementRouteWeight(self, weight):
        """Increment counter of weights travelled by packet"""
        self.total_route_weight += weight
        self.intermediate_route_weight += weight

    def decrement_ttl(self, current_time):
        self.ttl -= current_time - self.time
        self.time = current_time

    def set_current_node(self, node):
        """The current node the packet is located at."""
        self.current_node = node

    def clear_counters(self):
        """Restart intermediate counters"""
        self.intermediate_route_weight = 0
        self.intermediate_hops_travelled = 0
...
class AcknowledgementPacket(Packet):
    def __init__(self, time, size, id, src="a", dst="z", flow_id=0):
        super().__init__(time=time, size=size, src=src, dst=dst, flow_id=flow_id, id=id)
        self.acknowledgement_id = "ack{}".format(id)
...
class PacketGenerator(object):
    """ Generates packets with given inter-arrival time distribution.
        ...
        -----
        dstdist: function
            a no parameter function that returns the sucessive destination
    """
    def __init__(self, env, id,  adist, sdist, initial_delay=0, finish=float("inf"), flow_id=0, dstdist=None):
        self.id = id
        self.env = env
        self.adist = adist
        self.sdist = sdist
    ...
    def run(self):
        ...
        while self.env.now < self.finish:
            ...
            self.packets_sent += 1
            dst = None if self.dstdist() is None else self.dstdist()
...
class PacketSink(object):
    ...
    def put(self, pkt):
        ...
        if not self.selector or self.selector(pkt):
            ...
            self.total_weight += pkt.total_route_weight
            if self.debug:
                print(pkt)
...
class SwitchPort(object):
    ...
    """
    def __init__(self, env, rate, qlimit=None, limit_bytes=True, debug=False, id=None):
        self.store = simpy.Store(env)
        self.id = id
        self.rate = rate
        self.env = env
        ...
        self.size_packet = 0  # Current size of queue in packets
    ...
    def clear_packets(self):
        while 0 < self.size_packet or not len(self.store.items) == 0:
    ...
    def put(self, pkt):
        ...
        if pkt.ttl <= 0:
            self.packets_drop += 1
            print("Ttl expired, packet " + str(pkt.id) + " is dropped")
            return None
